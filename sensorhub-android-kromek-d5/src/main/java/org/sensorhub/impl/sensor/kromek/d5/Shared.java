...
import static org.sensorhub.impl.sensor.kromek.d5.reports.Constants.KROMEK_SERIAL_FRAMING_ESC_BYTE;
import static org.sensorhub.impl.sensor.kromek.d5.reports.Constants.KROMEK_SERIAL_FRAMING_ESC_ESC_BYTE;
...
public class Shared {
    ...
    static SerialReport sendRequest(SerialReport report, InputStream inputStream, OutputStream outputStream) throws IOException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        // Decode the received data using SLIP framing - Note: this doesn't appear to be necessary

        // The first five bytes are the header
        byte componentId = receivedData[3];
        byte reportId = receivedData[4];
        ...
        byte[] payload = Arrays.copyOfRange(receivedData, 5, receivedData.length - 2);
